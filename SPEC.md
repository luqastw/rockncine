...
mantêm o cuid de 25 caracteres e continuam válidas — ver seção 11, achado 6.
...
"tocando" pra sempre. Desde a rodada da seção 11 (achado 10), o badge "ao vivo" e o `SyncRing` leem
...
- **Utilitária (mono)** — pra código da sala, timestamp do chat, e qualquer diagnóstico de sync (`currentTime`, drift em ms). Mono aqui não é estética — é funcional: elimina ambiguidade `0`/`O`, `1`/`l` num código que a pessoa vai ditar ou colar pro amigo entrar na sala. **A mono sozinha não resolvia isso**: até a seção 11 o código era o cuid de 25 caracteres do Prisma, indititável na prática e sem botão de copiar em lugar nenhum. Hoje `lib/room-code.ts` gera 8 caracteres num alfabeto que já exclui `0/O`, `1/I/L` e `U` — a mono passa a reforçar uma decisão de formato, não a compensar sozinha um identificador impossível de ditar — e `components/room/InviteCode.tsx` copia a URL completa do convite ao lado do código (seção 11, achado 6).
...
player. **Aba/bottom-sheet foi cogitada e não é o mecanismo implementado** (seção 11, achado 9): o
...
5. **`backdrop-blur` sobre a área do vídeo — PLAUSÍVEL, custo de GPU.** `PlayerControls.tsx:42` e o
   ...
   **REVISADO na seção 11 (achado 3): o `invisible` foi desfeito, o fundo opaco continua.**
...
**REVISADO na seção 11 (achado 1): nessa configuração a barra mínima não some mais por
...
captura de ponteiro da seção 7 não pode ser montada aqui — ela bloquearia justamente o clique dentro
...
## 11. Auditoria UI/UX — rodada 2

Segunda passada de auditoria no projeto inteiro, pedida pelo usuário depois das correções da seção
10. Diferença de método em relação à rodada anterior: além de ler o SPEC.md e o código, esta rodada
...
o que a seção 7 diz querer esconder. Correção: camada transparente `absolute inset-0 z-10`
...
documentada como revisão na 9.7.

**2. Overlay de pausa do YouTube cobria a barra inteira — corrigido.** O overlay que esconde a tela
de sugestões do YouTube pausado (seção 7) era `absolute inset-0 z-10`, e a barra de controles era
...
escondido da barra usava `invisible` (decisão da 9.3, item 5), que remove os controles da ordem de
...
piso da seção 8 ("todo elemento interativo tem estado de foco visível, inclusive em navegação por
...
está registrada dentro da própria 9.3 pra ela não continuar lida como decisão vigente.
...
**Desvio:** a seção 8 previa "aba ou bottom sheet"; o que resolveu foi transformar a página numa
...
condições: sem rolagem de página, vídeo de 306px, log de 115px com `overflow-y: auto`. O texto da
seção 8 foi corrigido pra descrever o mecanismo real, e não a aba que não existe.
...
**12. Contorno de campos de formulário em `--line` — corrigido.** O item 7 da seção 10 trocou a
borda só nos botões. Nos inputs, `--line` (#2A2A2A) sobre `--bg-void` dá 1,36:1 e o preenchimento
...
de conexão/pausado (`RoomExperience.tsx:277` e `:283`). O chat já tinha `role="log"` desde a
seção 10.
...
**20. Título da sala em `text-sm` — corrigido.** A seção 8 reserva o papel Display pro título da
...
na seção 10 item 5, "Display" aqui é peso/tamanho/tracking sobre a mesma sans — não há família nova
instalada.
...
salas. O POST nativo com redirect 303 da seção 3 não mudou.
...
**25. Badge do Liveblocks colidindo com a barra de controles — corrigido.** A seção 6 escolheu
...
que era o "● ● ● 3" do esquema da seção 8 e nunca tinha sido implementado.
...
assinar presença no componente que renderiza a sala inteira (mesma preocupação de re-render da
9.3, item 2).
